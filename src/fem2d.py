...
def gen_necessary_data(
    ...
    Kloc = (a1m/(2*twodeltae))[:, np.newaxis, np.newaxis] * bj_outer + (a2m/(2*twodeltae))[:, np.newaxis, np.newaxis] * cj_outer + (betam*twodeltae/24)[:, np.newaxis, np.newaxis] * tmpmat[np.newaxis, :, :]
...
def main():
    ...
    tlist = np.array([[ 0, 4, 1], [3, 1, 4 ], [ 5, 0, 1], [3, 5, 2 ], [2, 1, 3], [5, 1, 2]]) # tlist will be outputted from gmsh, for now we just hardcode it
    dr = [0, 4, 3, 5]
