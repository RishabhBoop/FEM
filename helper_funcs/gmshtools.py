    ...
    def plot(self, ax=None, newaxis=False, color="grey", alpha=0.3, node_label=False, direction=False, *args, **kwargs):
        ...
        if self.elements.shape[1] > 1:
            if self.parent.dim == 3:
                ...
                poly = PatchCollection(
                    patches_list,
                    facecolor=lighten(color, 0.1),
                    edgecolor=color,
                    alpha=alpha,
                    linewidths=linw,
                    *args,
            ...
            if self.elements.shape[1] == 2 and direction == True:
                ...
                if self.parent.dim == 3:
                    ...
                    segments_2d = np.concatenate(
                        [np.stack([p2[:, :2], p1[:, :2]], axis=1), np.stack([p1[:, :2], p3[:, :2]], axis=1)], axis=0
                    )
        ...
        if node_label:
            ...
            for i in i_node:
                bx.text(
                    *self.parent.points[i, 0 : self.parent.dim], str(i), color="black", fontsize=9, fontweight="bold"
                )
    ...
    def select(self, new_name, fkt):
        ...
        if num == 2:
            setattr(
                self.parent,
                new_name,
                ElementMsh(self.parent, new_elements, info_text, node=True, connect=self.parent.edge_tab),
            )
        else:
    ...
    def __init__(self, mod, points=[], triangles=[], segments=[]):
        ...
        for elty in ElementTyp:
            ...
            if attr is not None:
                ...
                [
                    edge_tab.setdefault((int(ee[0]), int(ee[1])), []).append(elements[i // nn].astype(int).tolist())
        ...
        # Ränder
            ...
            for key in segments:
                setattr(
                    self,
                    key,
                    ElementMsh(self, np.array(segments[key]), "Defined by dict", node=True, connect=self.edge_tab),
                )
...
def FindLeftRight(name, segments, neighbor_elements):
    ...
    return (
        np.array(number_neighbors),
        np.array(left_idx),
        np.array(left_elements),
        np.array(right_idx),
        np.array(right_elements),
    )


#####
# Gebe jene zurück, bei denen das gebiet in richtung direction liegt
def OrientandResort(name, mesh, direction="left", consecutive=True):
    """
    Segments with a given direction (left or right) are extracted. If the segments are not continously connected (consecutive order),
    they are resorted.

    name:         name of the object (e.g. name of line in the mesh class)
    mesh:         mesh class
    direction:    domain may be 'left' or 'right'

    return
       for direction left: segments and connecting elements with domain on left side.
       The connecting elements are resorted and start with the first node of the corresponding segment

    """

    attr = getattr(mesh, name, None)
    if attr is not None:
        number_neighbors, l_idx, left_e, r_idx, right_e = FindLeftRight(name, attr.elements, attr.connect)

        if direction == "left":
            segments = attr.elements[l_idx]
            connecting_elements = left_e
        elif direction == "right":
            segments = attr.elements[r_idx]
            connecting_elements = right_e
        else:
            print("Only left and right possible.")
            return None
    else:
        print("No Element named: ", name)
        return None

    if (not np.array_equal(segments[1:, 0], segments[:-1, 1])) and consecutive == True:
        print("The single segments of the curve ", name, " are not in consecutive order, resort order ...")
        return SortConsecutive(segments, connecting_elements)

    else:
        return segments, connecting_elements


#####################
def SortConsecutive(seg, connect):
    """
    sort in consecutive order
    seg       : segments to be sorted
    connect   : connecting elements. If not known use []

    return resorted segments
    """

    k = seg.shape[1]

    # suche anfangspunkt
    start = np.setdiff1d(seg[:, 0], seg[:, 1])
    if len(start) == 0:
        print("closed curve")
        ind = 0
    elif len(start) > 1:
    ...
    if len(connect) != 0:
        arr = np.column_stack((seg, connect))
    else:
        arr = 1 * seg

    sorted_arr = arr[ind, :][None, :]
    np.delete(arr, ind, axis=0)

    # sortiere um, erstes element ist bereits festgelegt
    for i in range(len(seg) - 1):
        search_ind = sorted_arr[-1][k - 1]
        # suche nun passendes nächstes seg element
        for j in range(len(arr)):
            # gefunden
            if arr[j][0] == search_ind:
                sorted_arr = np.vstack([sorted_arr, arr[j, :]])
                np.delete(arr, j, axis=0)
                break

    if len(sorted_arr) != len(seg):
        print("Error, len(sorted_segments) != len(segments) ")
        return None

    return sorted_arr[:, :2], sorted_arr[:, 2:]


#######################
def NormalDerivative(p, sol, seg, connecting):
    """
    Berechne die Normalenableitung (nur für Linien). Der Normalenvektor zeigt in das Gebiet.

       seg        : segmente  (z.B. Linien)
       connecting : sortierte angrenzenden Elemente (z.b. Dreiecke)
       sol   : Lösung

       Die Segmente sollten fortlaufend sortiert sein z.b. [ [12,27], [27,19], [19,3], [3,.] ... ]
       Die connecting Elemente sind sortiert und müssen immer mit dem ersten Index des Segmentes beginnen
           links orientiert  : [ [12,27,9],[27,19,8],[3,...] ]
           rechts orientiert : [ [12,9,27],[27,8,19],[3,...] ]
       Die gewünschte Umlaufrichtung wird daraus ermittelt

       output:
       running length, normal derivative, gradient, normal vector
    ...
    # Finde Laufrichtung
    # elemente liegen links
    if (seg[0, 0] == n[0, 0]) and (seg[0, 1] == n[0, 1]):
        dir_fac = -1
        print("Umlaufsinn: Gebiet liegt links")
    else:
        dir_fac = 1
        print("Umlaufsinn: Gebiet liegt rechts")
    ...
    normal = dir_fac * np.column_stack([vec[:, 1] / vec_abs, -vec[:, 0] / vec_abs])
    ...
    return rl, normal_derivative, normal, grad_u
