  ...
  if n==1:
    ...
    for t in tri:
      ...
      for Q in [q1,q2,q3,q4]:
        N1=1./twodelta*(a1+b1*Q[0]+c1*Q[1])
        N2=1./twodelta*(a2+b2*Q[0]+c2*Q[1])
        N3=1./twodelta*(a3+b3*Q[0]+c3*Q[1])   
        Basefunc=[(2*N1-1)*N1,(2*N2-1)*N2,(2*N3-1)*N3,4*N1*N2,4*N2*N3,4*N1*N3]
...
def PlotMeshNumbers(p,t,edges=[],pltshow=True):
  ...
  for n in range(len(t)):
    buf='%i'%(n)
    ps=(p[t[n,0],:]+p[t[n,1],:]+p[t[n,2],:])/3.
    plt.text(ps[0],ps[1],buf,color='b',fontsize=9)

  if edges!=[]:
    for n in range(len(edges)):
      buf='%i'%(n)
      ps=(p[edges[n][0],:]+p[edges[n][1],:])/2.
...
#
def PlotBoundary(po , boundary,btype):
  """
...
def MakeSurfaceMesh(xstr,ystr,zstr,u0,u1,v0,v1,n1,n2):
  ...
  if N!=len(uv_bound):
    print("WARNING: Additional boundary points inserted by triangle.c (du too different from dv)")
  ...
  for i in range(N):
    ...
    for j in range(i+1,N):
      ...
      if bseg[j]>=0 and bseg[i]>=0 and np.sum((bound_p[i,:]-bound_p[j,:])**2)<eps:
        id+=(bseg[j],)
...
def MakeSphere(P0,R,mesh_len,epsilon=1e-8,type='cart'):
  ...
  if type=='cart':
    ...
    pp,tt=MakeSurfaceMesh(xstr,ystr,zstr,0,2*np.pi,np.pi/4.,np.pi/2.,Nu,int(Nu/3.))
   ...
   #make part of the sphere
    ...
    zstr='%g*np.cos(v)'%(R)
    Nu=int(2.*np.pi*R/mesh_len)
    pn,tn=MakeSurfaceMesh(xstr,ystr,zstr,0,2.*np.pi,0,np.pi,Nu,int(Nu/3.))
...
def ConnectMesh(p1,t1,b1,p2,t2,b2,epsilon=1e-8):
  ...
  boundary2=[ [ tab[x[0]] , tab[x[1]] ] for x in b2 if not (idn2.count(x[0])==1 and idn2.count(x[1])==1) ]       
  ...
  for k in range(len(Ps)-1):
    ...
    if last_bnr==first_bnr:
      ...
      if this_type>=len(NoSe):
        print("Error (this_type=%g , len(NoSe)=%g):  Increase  Nodes/Segments List"%(this_type,len(NoSe)))  
      if NoSe[this_type]=='Nodes': 
        ...
        if first<last:
          ...
          add=row1[first:curve_list[first_bnr+1]:]+row1[curve_list[first_bnr]:last+1]
          add+=row3[first:curve_list[first_bnr+1]:]+row3[curve_list[first_bnr]:last+1]
        # line not closed          
        if nn[k]!=nn[k+1]:
          add+=[row2[last]]          
      # segments     
        ...
        if first<last:
          ...
          add=segments[first:curve_list[first_bnr+1]:]+segments[curve_list[first_bnr]:last+1]
...
def FindBoundary(p,t,P1,P2,btype):
  ...
  rand_seg,rand_list,P1_boundary=ConnectBoundary(rand_seg,p,pstart=[(P1[0],P1[1])])  
...
def ContourSurface(p,t,u,iso_in,infig):
  ...
  for iso in isolines:
    ...
    iso_edges=[[i//3,tuple(x)] for i,x in enumerate(all_edges) if min(u[x])<=iso<=max(u[x])]
    ...
    if len(iso_edges)>1:      
      ...
      for k in range(len(nis)-1):      
        ...
        mal.plot3d(X,Y,Z,tube_radius=None,figure=infig,color=(0.2,0.2,0.2),line_width=5)
...
# 
def NormalDerivative(boundary,p,tt,u,inner=None,BouE=[],details=False):
  """
  ...
  Input:   boundary   [[n1,n2],[n3,n4],...]          boundary segments
           ...
           details                                   if True  the gradient the normal vector and 
                                                the elemental numbers are  additionally returned
  
  Output:  
            nor       normal derivative at the segments
           rl        running length
           line_int  line integral over boundary of the normal derivative
           dic  dictionary with details if details is True. 
                   dic['gradients'], dic['normals'] and dic['elements'] are provided 
  """
  
  if inner !=None and BouE!=[]:
    print("Error, if inner is set, BouE can not be set")
    return False
  ...
  if len(boundary[0])==3:
    ...
    for seg in boundary:
      ...
      X1=p[b_elem[0]]; X2=p[b_elem[1]]; X3=p[b_elem[2]] 
      ls=np.sqrt(np.sum((X2-X1)**2))
      
      b_1=X2[1]-X3[1]; b_2=X3[1]-X1[1]; b_3=X1[1]-X2[1]
      c_1=X3[0]-X2[0]; c_2=X1[0]-X3[0]; c_3=X2[0]-X1[0]       
      ...
      b12=b_1*b_2; b13=b_1*b_3; b23=b_2*b_3; b11=b_1**2; b22=b_2**2; b33=b_3**2
      c12=c_1*c_2; c13=c_1*c_3; c23=c_2*c_3; c11=c_1**2; c22=c_2**2; c33=c_3**2
      ...
      Qseq[3]=-(a_1*b23+a_1*c23+a_2*b13+a_2*c13+2*b12*b_3*xM+b13*c_2*yM+b_1*c23*xM+b23*c_1*yM+b_2*c13*xM+2*c12*c_3*yM)/delsq 
      Qseq[4]=-(a_2*b33+a_2*c33+a_3*b23+a_3*c23+2*b_2*b33*xM+b23*c_3*yM+b_2*c33*xM+b33*c_2*yM+b_3*c23*xM+2*c_2*c33*yM)/delsq 
      Qseq[5]=-(a_1*b33+a_1*c33+a_3*b13+a_3*c13+2*b_1*b33*xM+b13*c_3*yM+b_1*c33*xM+b33*c_1*yM+b_3*c13*xM+2*c_1*c33*yM)/delsq
      
      
      density=Qseq[0]*Phi_1+Qseq[1]*Phi_2+Qseq[2]*Phi_3+Qseq[3]*Phi_4+Qseq[4]*Phi_5+Qseq[5]*Phi_6
      line_int+=density
    ...
    if details==True:
      det_dict=dict()
      det_dict['elements']=np.array(elements)
      det_dict['normals']=np.array(normals)
      det_dict['gradients']=np.array(gradients)
      return np.array(nor),np.array(rl),line_int,det_dict
    else:
      return np.array(nor),np.array(rl),line_int    
  ...
  for s in boundary:
    ...
    if len(no_el)==2:      
      ...
      if (he<0 and inner=='right') or (he>0 and inner=='left'):
        n=tt[no_el[0][1],:]
        # details, element number
        elements += [  tt[no_el[0][1]]  ]   
      else:
        n=tt[no_el[1][1],:]  
        elements += [ tt[no_el[1][1]]  ]
    ...
    #details
    normals +=  [  [ normal_x/Ls , normal_y/Ls ]  ] 
    ...
    dudn=0.5/Ls/delta
    gradients += [ [ 0.5/delta*phi_x , 0.5/delta*phi_y ] ]
  ...
  if details==True:
    ...
    det_dict['normals']=np.array(normals)
    det_dict['gradients']=np.array(gradients)
    return nor,rl,line_int,det_dict
  else:
...
def FindNextSegment(all_segments,node):
  ...
  if len(help)==0: #if connecting segment does not exist (=>new boundary)
    ...
    one_occ=[[i//2,x] for i,x in enumerate(rest_nodes) if rest_nodes.count(x)==1]
...
def ConnectBoundary(boundary_segments,Pall,pstart=[]):
  ...
  #print("Rückgabe: ",allseg,boundaries)  
...
# Rectangle
def RectangleSegments(P1,P2,num_points=60,edge_length=-1,edge_lengthx=-1,edge_lengthy=-1):
  """
...
def ORecSegments(P1,P2,rho,num_points=60,num_pc=7,edge_length=-1,edge_lengthx=-1,edge_lengthy=-1):
  """
  p,v = ORecSegments(P1,P2,rho,num_points=60,num_pc=7,edge_length=-1,edge_lengthx=-1,edge_lengthy=-1)
  """
  x1L=P1[0];          x2L=P2[0]-rho
  x1R=P1[0]+rho;  x2R=P2[0]

  y1L=P1[1]+rho;    y3L=P2[1]
  y1R=P1[1];           y3R=P2[1]-rho    
  ...
  p_5,v_5=CircleSegments([x2L,y1L],rho,num_pc,-np.pi/2.,0,edge_length)
  p_6,v_6=CircleSegments([x2L,y3R],rho,num_pc,0,np.pi/2.,edge_length)
  p_7,v_7=CircleSegments([x1R,y3R],rho,num_pc,np.pi/2.,np.pi,edge_length)
  p_8,v_8=CircleSegments([x1R,y1L],rho,num_pc,np.pi,3*np.pi/2,edge_length)
...
def PointSegments(p,edge_length=-1):
  ...
  delta=np.min(np.sqrt(np.sum((p1[1:]-p1[:-1])**2,axis=1)))/10.
...
# Generate mesh  
def DoTriMesh(points,vertices,edge_length=-1,holes=[],tri_refine=None,show=True,order=None,writeTo=None):
  """
  ...
  if tri_refine!=None:
    ...
    mesh = triangle.build(info,max_volume=0.5*edge_length**2,mesh_order=order,generate_faces=True)
  ...
  # mesh_edges = np.array(mesh.faces,  dtype=np.int)     
  ...
  edges=[tuple(x) for x in mesh_facets if len(edge_element_map[tuple(np.sort(x))])==2]
  ...
  if order==2:
    ...
    third_node=FindClosestNode(MiddleIndices,mesh_points,ps,constraint=-1,tree=None)
    bou_Edges=[[x[0],x[1],third_node[0][i]] for i,x in enumerate(bou_Edges)]
    
    
    err=CheckElementList(mesh_points,mesh_elements)
    if err>0:
      print("ERROR: Number of wrong sorted mesh_elements ",err,"# mesh_elements ",len(mesh_elements))    
      return False
  
  if writeTo!=None:
    np.savez(writeTo,mesh_points,mesh_elements,bou_Edges,list_bE,mesh_boundary_elements,Curves,list_Cu)  
  
  return mesh_points,mesh_elements,bou_Edges,list_bE,mesh_boundary_elements,Curves,list_Cu
...
def WriteXmlMesh(filename,p,t,do=None):
  ...
  #root element
  dolf = ET.Element('dolfin', {'xmlns:dolfin':'https://fenicsproject.org converted from meshpy and meshtools'})
  ...
  for i,pp in enumerate(p):
    line = ET.SubElement(vert, 'vertex', {'index':str(i), 'x': str(pp[0]), 'y':str(pp[1])})
  
  
  # elements
  cells = ET.SubElement(mesh, 'cells', { 'size': str(len(t)) })
  for i,tt in enumerate(t):
    line = ET.SubElement(cells, 'triangle', {'index':str(i), 'v0':str(tt[0]), 'v1':str(tt[1]) , 'v2':str(tt[2])})
  ...
  if do == None:
    do = len(t)*[0]  
  msh_vc = ET.SubElement(dom, 'mesh_value_collection', {'name':'m' , 'type':'uint' , 'dim':'2' , 'size':'686'}) 
  for i,dd in enumerate(do):
    line = ET.SubElement(msh_vc, 'value', {'cell_index':str(i), 'local_entity':'0', 'value':str(dd)})
...
def CheckElementList(p,t):
  ...
  if len(t[0])==3:
    ...
    for i in range(len(t)):
      diff1=np.sum( ( (p[t[i,0]]+p[t[i,1]])/2.-p[t[i,3]])**2 )
      diff2=np.sum( ( (p[t[i,1]]+p[t[i,2]])/2.-p[t[i,4]])**2 )
      diff3=np.sum( ( (p[t[i,2]]+p[t[i,0]])/2.-p[t[i,5]])**2 )
...
def DoBemBoundary(AllBound,show=True,show_numbers=False):
  ...
  for i in range(NofC):
    ...
    count=0
    for s in v:
      VecB[Curves[i]["first"]+count]=np.array([0.5*(p[s[1],:]-p[s[0],:])])
      VecXm[Curves[i]["first"]+count]=np.array([0.5*(p[s[1],:]+p[s[0],:])])
      count+=1
...
def DoBemDomain(AllDom,CC):
  ...
  for i in range(len(CC)):
    Curves[i]={"first": CC[i]["first"],"last": CC[i]["last"],"type": CC[i]["type"]}
