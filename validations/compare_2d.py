...
def gen_mesh():
    ...
    formula = f"0.00001 + 0.1 * F{dist_tag}"  # 0.0005 is the minimum element size, 0.5 controls how fast the size grows with distance
...
def cpp_no_mkl(netz):
    ...
    fem_solver = fem_cpp.FEM_2D(dd, rr, plist, tlist, alpha1, alpha2, beta, f, phi, gamma, q)
    try:
        timings = fem_solver.full_solve()
        sol = fem_solver.get_Solution()
        visualize_solution(
            plist, tlist, sol, False, "2D FEM no MKL", False, "CPP", f"Loesung2D_{len(plist)}points_sol.png"
        )
        print_timings(timings, "FEM 2D Timings without MKL", len(plist), len(tlist), False, "CPP")
...
def cpp_mkl(netz):
    ...
    fem_solver = fem_cpp_mkl.FEM_2D(dd, rr, plist, tlist, alpha1, alpha2, beta, f, phi, gamma, q)
    try:
        timings = fem_solver.full_solve()
        sol = fem_solver.get_Solution()
        visualize_solution(
            plist, tlist, sol, False, "2D FEM with MKL", False, "CPP", f"Loesung2D_{len(plist)}points_sol.png"
        )
        print_timings(timings, "FEM 2D Timings with MKL", len(plist), len(tlist), False, "CPP")
    except RuntimeError as e:
        print(f"Fehler: {e}")
        return
